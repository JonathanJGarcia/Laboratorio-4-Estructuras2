...
class Program
{
    ...
    private static string conversationsFolderPath = @"D:\visual studi\2do semestre 2024\Lab4 estructuras 2\inputs";

    static void Main()
    {
        LoadCommandsFromFile(@"D:\visual studi\2do semestre 2024\Lab4 estructuras 2\input.csv");
        UserSearch();
    }
    ...
    private static void UserSearch()
    {
        ...
        while ((dpi = Console.ReadLine()) != "exit")
        {
            ...
            if (user != null)
            {
                ...
                Console.WriteLine($"Registro encontrado:\n{JsonConvert.SerializeObject(displayUser, Newtonsoft.Json.Formatting.Indented)}");

                Console.WriteLine("Ingresa la contraseña para cifrar/descifrar las conversaciones: ");
                string password = Console.ReadLine();
                DisplayConversations(decodedDpi, password);
            }
            else
            {
    ...
    private static void DisplayConversations(string dpi, string password)
    {
        var conversationFiles = Directory.GetFiles(conversationsFolderPath, $"CONV-{dpi}-*.txt");

        foreach (var filePath in conversationFiles)
        {
            string content = File.ReadAllText(filePath);
            string encryptedContent = TranspositionCipher.Encrypt(content, password);
            string decryptedContent = TranspositionCipher.Decrypt(encryptedContent, password);

            Console.WriteLine($"Archivo: {Path.GetFileName(filePath)}");
            Console.WriteLine($"Contenido Cifrado: {encryptedContent}");
            Console.WriteLine($"Contenido Descifrado: {decryptedContent}");
        }
    }
...
public static class TranspositionCipher
{
    public static string Encrypt(string text, string key)
    {
        ...
        int numRows = (int)Math.Ceiling((double)textLength / keyLength);
        ...
        for (int row = 0; row < numRows; row++)
        {
            ...
            {
                grid[row, col] = index < textLength ? text[index++] : ' ';
            }
        }
